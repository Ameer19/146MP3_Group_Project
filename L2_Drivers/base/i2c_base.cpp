        ...
        mpI2CRegs(pI2CBaseAddr), mDisableOperation(false)
{
    ...
    vTraceSetMutexName(mI2CMutex, "I2C Mutex"); vTraceSetSemaphoreName(mTransferCompleteSignal, "I2C Finish Sem");

    //xSemaphoreTake()//Task semaphore here
    ...
    mRegisters = NULL;
    mRegisterSize = 0;
    mTransaction.bytesRx = 0;
    mTransaction.bytesTx = 0;
...
bool I2C_Base::initSlave(uint32_t slaveAddr, uint8_t *regAddr, uint32_t size)
{
    Master1Slave0 = 0;
    mRegisters = regAddr;
    mRegisterSize = size;
    ...
    //Enable I2C Slave Reciever mode
    mpI2CRegs->I2CONSET = 0x44;

    //Enable I2C Interrupts based off mIRQ
    NVIC_EnableIRQ(mIRQ);
...
I2C_Base::mStateMachineStatus_t I2C_Base::i2cStateMachine()
{
    ...
    //I2CONCLR is write-only. Writing a 1 to a bit of this registers causes the corresponding bit in the I2C control register to be cleared.
...
#define setStop()           clearSTARTFlag();                           \
    ...
    switch (mpI2CRegs->I2STAT) {
        case start:
            mpI2CRegs->I2DAT = I2C_WRITE_ADDR(mTransaction.slaveAddr); //loading data into I2DAT.
            clearSIFlag(); // Enable I2C Interrupt Clear bit. When SCL is high, it is unaffected by the state of the SI flag. This bit must be rest by software with I2CCONCLR->SI |= (1<<3) . I2CCONSET->SI (bit 3) should be cleared after the required bits have been set and the value in I2DAT has been loaded or read.
            //uart0_puts("Master State: start\n");
            break;
        case repeatStart:
            mpI2CRegs->I2DAT = I2C_READ_ADDR(mTransaction.slaveAddr); //loading data into I2DAT.
            clearSIFlag();
            //uart0_puts("Master State: repeatStart\n");
            break;

        case slaveAddressAcked:
            clearSTARTFlag(); //clear the start flag. slave as acked.
            ...
            if (0 == mTransaction.trxSize) {
                ...
                mpI2CRegs->I2DAT = mTransaction.firstReg; //load first register address you wanna start writing from M.
                clearSIFlag();
            }
            //uart0_puts("Master State: slaveAddressAcked\n");
            break;

        case dataAckedBySlave:
            if (I2C_READ_MODE(mTransaction.slaveAddr)) { //can only use read mode from an ODD address.
                ...
                if (0 == mTransaction.trxSize) {
                    setStop()
                    ; //Basically polls for I2CONSET bit 4 to ensure the stop flag is 1. Then based on the address we know a write(even) or read(odd) is complete
                }
                else {
                    mpI2CRegs->I2DAT = *(mTransaction.pMasterData);
                    ++mTransaction.pMasterData; //Go to next data location
                    --mTransaction.trxSize; //decrease the transmit buffer size
                    clearSIFlag();
                }
            }
            //uart0_puts("Master State: dataAckedBySlave\n");
            break;
        ...
        case readAckedBySlave:
            ...
            clearSIFlag();
            uart0_puts("Master State: readAckedBySlave\n");
            break;
        case dataAvailableAckSent:
            *mTransaction.pMasterData = mpI2CRegs->I2DAT;
            ...
            clearSIFlag();
            //uart0_puts("Master State: dataAvailableAckSent\n");
            break;
        case dataAvailableNackSent: // Read last-byte from Slave
            *mTransaction.pMasterData = mpI2CRegs->I2DAT;
            setStop();
            //uart0_puts("Master State: dataAvailableNackSent\n");
            break;
        ...
        case arbitrationLost:
            ...
            //uart0_puts("Master State: arbitrationLost\n");
            break;
...
// State Machine code for slave machine operation
I2C_Base::mStateMachineStatus_t I2C_Base::i2cSlaveStateMachine()
{

    enum {
        //rt - returned, tx - transmitted, rx - received, AL_ - Arbitration Lost in sla+r/w as master
        ...
        SLA_Wrx_ACKrt = 0x60,
        AL_SLA_Wrx_ACKrt = 0x68,
        GCrx_ACKrt = 0x70,
        ALGCrx_ACKrt = 0x78,
        DATArx_ACKrt = 0x80,
        DATArx_NACKrt = 0x88,
        GC_DATArx_ACKrt = 0x90,
        GC_DATArx_NACKrt = 0x98,
        StopForSlaveRecieverRepeatStartForSlaveTransmitter = 0xA0
    };
    ...
    switch (mpI2CRegs->I2STAT) {
        //Slave Transmitter
        case SLA_R_ACKrt:
            count = mTransaction.firstReg;
            mpI2CRegs->I2DAT = mRegisters[count];
            mTransaction.bytesTx = 1;
            setAckFlag();
            clearSIFlag();
            uart0_puts("Slave State: SLA_R_ACKrt");
            break;

        case AL_SLA_R_ACKrt:
            setNackFlag();
            clearSIFlag();
            uart0_puts("Slave State: AL_SLA_R_ACKrt\n");
            break;

        case I2DATtx_ACKrx:
            count = mTransaction.firstReg + mTransaction.bytesTx;
            count = count % mRegisterSize;
            mpI2CRegs->I2DAT = mRegisters[count];
            mTransaction.bytesTx++;
            clearSIFlag();
            uart0_puts("Slave State: I2DATtx_ACKrx\n");
            break;
        ...
        case I2DATtx_NACKrx:
            mTransaction.bytesTx = 0;
            clearSIFlag();
            uart0_puts("Slave State: I2DATtx_NACKrx\n");
            break;
        ...
        case GCrx_ACKrt:
            setAckFlag();
            clearSIFlag();
            uart0_puts("Slave State: GCrx_ACKrt\n");
            break;

        case SLA_Wrx_ACKrt://60
            mTransaction.slaveAddr = mpI2CRegs->I2DAT;
            setAckFlag();
            clearSIFlag();
            uart0_puts("Slave State: SLA_Wrx_ACKrt\n");
            break;

        case AL_SLA_Wrx_ACKrt:
            setNackFlag();
            clearSIFlag();
            uart0_puts("AL_SLA_Wrx_ACKrt\n");
            break;

        case DATArx_ACKrt:
            mTransaction.bytesRx++;
            if (mTransaction.bytesRx == 1) {
                mTransaction.firstReg = mpI2CRegs->I2DAT;
            }
            else {
                count = mTransaction.firstReg + mTransaction.bytesRx - 2;
                count = count % mRegisterSize; //registers at end. Now goto 1st reg.
                mRegisters[count] = mpI2CRegs->I2DAT;
            }
            setAckFlag();
            clearSIFlag();
            uart0_puts("Slave State: DATArx_ACKrt\n");
            break;
        ...
        case DATArx_NACKrt:
            ...
            setNackFlag();
            clearSIFlag();
            uart0_puts("Slave State: DATArx_NACKrt\n");
            break;

        case GC_DATArx_ACKrt:
            mTransaction.bytesRx++;
            if (mTransaction.bytesRx == 1) {
                mTransaction.firstReg = mpI2CRegs->I2DAT;
            }
            ...
            setAckFlag();
            clearSIFlag();
            uart0_puts("Slave State: GC_DATArx_ACKrt\n");
            break;

        case GC_DATArx_NACKrt:
            mTransaction.bytesRx = 0;
            setNackFlag();
            clearSIFlag();
            uart0_puts("Slave State: GC_DATArx_NACKrt\n");
            break;

        case StopForSlaveRecieverRepeatStartForSlaveTransmitter:
            mTransaction.bytesRx = 0;
            mTransaction.bytesTx = 0;
            setAckFlag();
            clearSIFlag();
            uart0_puts("Slave State: StopForSlaveRecieverRepeatStartForSlaveTransmitter\n");
            break;
